<template>
    <div class="container card mt-5 ms-2">
      <div class="row justify-content-center">
        <div class="col-md-12">
          <div class="card-header mt-5">
            <h4 class="table_heading">Add Loan Proposal</h4>
          </div>
          <div class="card-body">
            <div class="row">
              <div class="col-md-6">
                <div class="mb-3">
                  <label for="customerName" class="form-label">Customer Name</label>
                  <select v-model="selectCustomer" class="form-select col-md-10">
                    <option value="">Customer Name</option>
                    <option v-for="(d, i) in customer" :key="i" :value="d.id">{{ d.customer_name }}</option>
                  </select>
                  <p style="color:red" v-if="customerError">{{ customerError }}</p>
                </div>
                <div class="mb-3">
                  <label for="loanType" class="form-label">Loan Type</label>
                  <select class="form-select col-md-10" v-model="selectLoanType">
                    <option disabled value="">Please Select the Loan Type</option>
                    <option v-for="(d, i) in loanType" :key="i" :value="d.id">{{ d.loan_type }}</option>
                  </select>
                  <p style="color:red" v-if="loanTypeError">{{ loanTypeError }}</p>
                </div>
                <div class="mb-3">
                  <label for="amount" class="form-label">Amount</label>
                  <input type="text" v-model="amount" class="form-control" placeholder="Enter Loan Proposal Amount">
                  <p style="color:red" v-if="amountError">{{ amountError }}</p>
                </div>
              </div>
              <div class="col-md-6">
                <div class="mb-3">
                  <label for="date" class="form-label">Date</label>
                  <input v-model="date" type="date" class="form-control" placeholder="Enter Deposit Date">
                  <p style="color:red" v-if="dateError">{{ dateError }}</p>
                </div>
                <div class="mb-3">
                  <label for="tenure" class="form-label">Tenure (Month)</label>
                  <input type="text" v-model="tenure" class="form-control" placeholder="Enter Tenure">
                  <p style="color:red" v-if="tenureError">{{ tenureError }}</p>
                </div>
              </div>
            </div>
            <button @click="saveLoanProposal" type="submit" class="btn btn-primary">Submit</button>
            <div v-if="successMessage" class="alert alert-success" role="alert">
              {{ successMessage }}
            </div>
          </div>
        </div>
      </div>
    </div>
  </template>
  
  <script>
  import axios from 'axios';
  
  export default {
    data() {
      return {
        url: 'http://127.0.0.1:8000/api/loanProposal',
        customer: [],
        loanType: [],
        selectCustomer: '',
        selectLoanType: '',
        amount: '',
        date: '',
        tenure: '',
        customerError: '',
        loanTypeError: '',
        amountError: '',
        dateError: '',
        tenureError: '',
        successMessage: '',
      };
    },
    mounted() {
      this.getCustomer();
      this.getLoanType();
    },
    methods: {
      getCustomer() {
        axios.get('http://127.0.0.1:8000/api/customer')
          .then(res => {
            this.customer = res.data.data;
          })
          .catch(error => {
            console.error('Error fetching customer', error);
          });
      },
      getLoanType() {
        axios.get('http://127.0.0.1:8000/api/loanType')
          .then(res => {
            this.loanType = res.data.data;
          })
          .catch(error => {
            console.error('Error fetching loanType', error);
          });
      },
      saveLoanProposal() {
        const allData = {
          customer_id: this.selectCustomer,
          loan_type_id: this.selectLoanType,
          amount: this.amount,
          date: this.date,
          tenure: this.tenure,
        };
  
        axios.post(this.url, allData)
          .then(res => {
            this.successMessage = 'Form submitted successfully!';
            setTimeout(() => {
              this.successMessage = '';
              this.$router.push({ name: 'CloanProposalAdd' });
            }, 3000);
          })
          .catch(error => {
            console.error('Error saving loan proposal', error);
          });
      },
      clearErrors() {
        this.customerError = '';
        this.loanTypeError = '';
        this.amountError = '';
        this.dateError = '';
        this.tenureError = '';
      },
    },
  };
  </script>
  
  <style>
  /* Add some basic styling */
  .alert {
    margin-top: 20px;
  }
  </style>
  